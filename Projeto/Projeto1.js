...
console.log('\n\t\t\t\tA JORNADA DO HERÓI\n\n');
prompt('\tEste é um jogo iterativo! Pressione ENTER para avançar com a história...');
while (respostas[0] != 'sim' && respostas[0] != 'nao') {                                                                        //confere se a resposta foia com 'sim' ou 'nao'
    respostas[0] = String(prompt('\tQuando houver uma pergunta responda apenas com SIM ou NAO. Entendido? ')).replace(/\s/g, '').toLowerCase(); 
...
prompt('\tEle encosta as mãos no chão e DISPARA a correr com muita velocidade...');

// PERGUNTA 1 - Você corre com toda sua velocidade para tentar escapar de Shia LaBeouf?
while (respostas[0] != 'sim' && respostas[0] != 'nao') {                                                                        //confere se a resposta foia com 'sim' ou 'nao'
    respostas[0] = String(prompt('\n[PERGUNTA 1] - Você corre com toda sua velocidade para tentar escapar de Shia LaBeouf? ')).replace(/\s/g, '').toLowerCase(); 
}                                                                                                                               //trata entrada -> string, sem espaços, minúsculo          
...
prompt('\tMeu Deus, há sangue em TUDO!!!');
...
while (respostas[1] != 'sim' && respostas[1] != 'nao') {                                                                        //confere se a resposta foia com 'sim' ou 'nao'
    respostas[1] = String(prompt('\n[PERGUNTA 2] - A única solução é cortar sua perna com os dentes! Você tenta fazer isso em silêncio? ')).replace(/\s/g, '').toLowerCase(); 
} 
...
prompt('\tVocê está se esgueirando atrás dele, a superestrela do estrangulamento Shia LaBeouf');
prompt('\tLutando por sua vida com...');
prompt('\t\t\t\t\t\t\t\t\t\tShia LaBeouf!');
prompt('\tTentando roubar a faca do...');
prompt('\t\t\t\t\t\t\t\t\t\tShia LaBeouf!');
prompt('\tVocê CONSEGUE esfaqueá-lo em seu RIM! Finalmente a salvo de Shia LaBeouf.');
// PERGUNTA 3 - Finalmente a salvo você lembra do seu carro. Você procura a chave antes de sair em busca dele?
while (respostas[2] != 'sim' && respostas[2] != 'nao') {                                                                        //confere se a resposta foia com 'sim' ou 'nao'
    respostas[2] = String(prompt('\n[PERGUNTA 3] - Finalmente a salvo você lembra de seu carro. Você procura a chave antes de sair em busca dele? ')).replace(/\s/g, '').toLowerCase(); 
}
prompt('\n\tAgora você manca pela floresta escura...');
prompt('\tSangue escorrendo de sua perna mutilada. Você derrotou Shia LaBeouf.');
prompt('\t\t\t\t\t\t\t\t\t\t    Espere!!!');
...
while (respostas[3] != 'sim' && respostas[3] != 'nao') {                                                                        //confere se a resposta foia com 'sim' ou 'nao'
    respostas[3] = String(prompt('\n[PERGUNTA 4] - Você ENTRA em uma luta corporal com Shia LaBeouf novamente? ')).replace(/\s/g, '').toLowerCase(); 
}
...
// PERGUNTA 5 - Você teria coragem de desferir o golpe final e matar Shia LaBouf?
while (respostas[4] != 'sim' && respostas[4] != 'nao') {                                                                        //confere se a resposta foia com 'sim' ou 'nao'
    respostas[4] = String(prompt('\n[PERGUNTA 5] - Você tem coragem e força para desferir o golpe final e matar Shia LaBouf? ')).replace(/\s/g, '').toLowerCase(); 
}
...
//FINAIS
if (conta_sim > 4) {                                                                                                            // se conta_sim = 5 => FINAL : TRIUNFO 
    prompt('\n\tVocê está cortando a cabeça dele agora...');
    prompt('\tVocê acaba de decapitar...');
    prompt('\t\t\t\t\t\t\t\t\t\tShia LaBeouf!');
    prompt('\tSua cabeça cai no chão, sem expressão...');
    prompt('\tVocê está finalmente a salvo de Shia LaBeouf.');
} else if (conta_sim > 3) {                                                                                                     // se conta_sim = 4 => FINAL : Vitória PARCIAL
...
} else if (conta_sim > 2) { 	                                                                                                // se conta_sim = 3 => FINAL : DERROTA HONRADA
    prompt('\n\tShia LaBeouf te bate com o cabo do machado...');
    prompt('\tVocê cai no chão inconsciente...');
    prompt('\tVocê acorda numa cela fria, agora você é um prisioneiro(a) de...');
    prompt('\t\t\t\t\t\t\t\t\t\tShia LaBeouf!');
    prompt('\tRecupere suas forças para fugir e ficar a salvo de Shia LaBeouf.');
} else if (conta_sim > 0) {                                                                                                     // se conta_sim = 1 ou 2 => FINAL : DERROTA 
    prompt('\n\tShia LaBeouf te enforca até você perder a conciência...');
    prompt('\tVocê acorda na floresta, em frente a seu carro. A chave está à sua direita...');
    prompt('\tÀ sua esquerda um papel contendo as palavras: "Você não é digno..." Assinado: ');
    prompt('\t\t\t\t\t\t\t\t\t\tShia LaBeouf!');
    prompt('\tSe não for uma pegadinha você foi humilhado mas está a salvo de Shia LaBeouf.');
} else {                                                                                                                        // se conta_sim = 0 => FINAL : DERROTA MISERAVEL
